...
testing_data_loader = DataLoader(dataset=testset, num_workers=args.threads, batch_size=args.testBatchSize,
                                 shuffle=False)
#print('training_data_loader',len(training_data_loader))
...
model = esrt.ESRT(upscale = args.scale, n_channels=args.n_colors)#architecture.IMDN(upscale=args.scale)
...
def forward_chop(model, x, scale, shave=10, min_size=60000):
    ...
    if w_size * h_size < min_size:
        sr_list = []
        #print('w_size * h_size < min_size')
        for i in range(0, 4, n_GPUs):
            lr_batch = torch.cat(lr_list[i:(i + n_GPUs)], dim=0)
            #print('lr_batch_shape :',lr_batch.shape)
            sr_batch = model(lr_batch)
            #print('sr_batch_shape :',sr_batch.shape)
    ...
    shave *= scale
    aa = sr_list[0][:, :, 0:h_half, 0:w_half]
    #print('sr_list_shape :',aa.shape)
