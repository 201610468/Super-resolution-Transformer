...
def compute_ssim(im1, im2):
    isRGB = len(im1.shape) == 1 and im1.shape[-1] == 1
    win_size = min(im1.shape)
    s = ssim(im1, im2, K1=0.01, K2=0.03, win_size=win_size, gaussian_weights=True, sigma=1.5, use_sample_covariance=False,
             multichannel=False)#isRGB
    return s
